criaCartao(
    'Biotecnologia',
    'O que é biotecnologia?',
    'A biotecnologia é um campo amplo que usa organismos vivos, ou componentes deles, para desenvolver produtos, processos e tecnologias.'
)

criaCartao(
    'Geografia',
   'Em que continente se encontra o maior deserto do mundo?',
    'a Antártida, encontra-se no continente antártico'
)

criaCartao(
    'Históŕia',
    'Que país tem o formato de uma bota? ',
    'A Itália é o país que tem o formato de uma bota'
)

criaCartao(
    'Física',
    'A que temperatura a água ferve?',
    '100 ºC é conhecido como o ponto de ebulição da água.'
)